        ...
        $joinedDate = date("Y/m/d");
				foreach($_FILES['images_upload']['name'] as $key=>$val)
            {
                 ...
                 if(is_uploaded_file($_FILES['images_upload']['tmp_name'][$key]))
                    ...
                    if ($insert && $insert2){
           						$_SESSION['success'] = '';
           					echo "<script type='text/javascript'>dialog('Thank you for being part of Tahfiz Care. We need 24 hours from now to verify your certificate before you can login.');
