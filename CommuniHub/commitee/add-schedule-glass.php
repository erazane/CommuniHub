...
<div class="main-content">
    <div class="wrapper">
        ...
        <tr>
                    <td colspan="2">
                    <input type="submit" name="SubmitGlassSchedule" value="Submit Schedule" class="btn-primary"/>
...
if (isset($_POST['SubmitGlassSchedule'])) {
    require_once('../Database/database.php');

    // Validate and assign Garbage Day
    if (!empty($_POST['GlassPickupday'])) {
        $GlassPickupday = $_POST['GlassPickupday'];
    } else {
        $errors[] = 'Must select a day.';
    }

    // Validate and assign Garbage Pickup Hour
    if (!empty($_POST['GlassPickupHour'])) {
        $GlassPickupHour = $_POST['GlassPickupHour'];
    } else {
        $errors[] = 'Must select an hour.';
    }

    // Validate and assign Garbage Pickup Minute
    if (!empty($_POST['GlassPickupMinute'])) {
        $GlassPickupMinute	 = $_POST['GlassPickupMinute'];
    } else {
    ...
    if (empty($errors)) {
        $query = "INSERT INTO glassSchedule (GlassPickupday, GlassPickupHour, GlassPickupMinute	) 
                  VALUES ('$GlassPickupday', '$GlassPickupHour', '$GlassPickupMinute	')";
