...
if (isset($_POST['SubmitPaperSchedule'])) {
    ...
    if (!empty($_POST['PaperPickupDay'])) {
        $PaperPickupDay = $_POST['PaperPickupDay'];
    } else {
        $errors[] = 'Must select a day.';
    }

    // Validate and assign Garbage Pickup Hour
    if (!empty($_POST['PaperPickupHour'])) {
        $PaperPickupHour = $_POST['PaperPickupHour'];
    } else {
        $errors[] = 'Must select an hour.';
    }

    // Validate and assign Garbage Pickup Minute
    if (!empty($_POST['PaperPickupMinute'])) {
        $PaperPickupMinute	 = $_POST['PaperPickupMinute'];
    } else {
    ...
    if (empty($errors)) {
        $query = "INSERT INTO paperSchedule (PaperPickupDay, PaperPickupHour, PaperPickupMinute	) 
                  VALUES ('$PaperPickupDay', '$PaperPickupHour', '$PaperPickupMinute	')";
